...
public class UserController {
    ...
    @GetMapping("/balance")
    public ResponseEntity<BalanceResponse> userBalance(@RequestParam("user_id") int userId) {
        BalanceResponse balanceResponse = new BalanceResponse();
        try {
            BalanceDTO balanceDTO = balanceService.userBalance(userId);
             balanceResponse.setSuccess(true);
             balanceResponse.setMessage("Successfully showing user balance");
            ...
            balanceResponse.setMessage("Failed to showing user balance");
            balanceResponse.setErrorCode("400");
